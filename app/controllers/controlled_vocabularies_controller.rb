class ControlledVocabulariesController < ApplicationController
  ...
  def create
    authorize ControlledVocabulary
    model_type = ControlledVocabulary.where(model_attribute: params[:controlled_vocabulary][:model_attribute]).pluck(:model_type).distinct.first
  ...
  def get_map
    @map = {}
    ControlledVocabulary.where("model_type like 'Title%'").pluck(:model_type).distinct.each do |mt|
      ControlledVocabulary.where(model_type: mt).pluck(:model_attribute).distinct.each do |ma|
